# Michele Grimaldi — Software Engineer
## C#/.NET · C++ · Python

Software Engineer with a background in game development and audio systems, pivoting toward backend and systems engineering. Specialized in C#/.NET architectures, event-driven systems, and high-performance tooling.

Open to remote roles and relocation across EU. Target market: software engineering, game dev, iGaming.

---

## Stack

**Primary:** C#, C++, Java *(in progress)*
**Secondary:** Python
**Backend:** ASP.NET Core, Spring Boot *(in progress)*, Django, PostgreSQL
**Game/Audio:** Unity, Unreal Engine, JUCE, Wwise
**Systems:** Event-Driven Architecture, Observer Pattern, Lock-free Multithreading, Memory Pooling, MVVM
**Workflow:** Git, Docker, CI/CD, Unit Testing, Linux, Agile/Scrum

---

## Projects

### Flagship
- **[DEAD AIR](https://m1k3-s0und.itch.io/dead-air-911)** — Narrative horror game built around 911 operator calls. Tag-driven C# architecture with 14 decoupled ScriptableObject Event Channels, Observer Pattern, Ink integration. Performance: <1ms CPU/frame, ~2MB memory at idle. Submitted to (No) Dialogue Jam 2026.

### Gameplay Systems & Architecture
- **[City Builder Prototype](https://github.com/Mike014/City-Builder-Prototipo-Unity)** — Command Pattern with undo/redo stack, Event Bus architecture, O(1) spatial lookup via flattened 1D array.
- **[Command Pattern Game](https://github.com/Mike014/Command_Pattern_Game)** — Turn-based game demonstrating Command Pattern with execute/undo on a 10x10 grid.
- **[Stealth Isometric](https://github.com/Mike014/M5_Final_Project)** — 5-state FSM enemy AI, NavMesh navigation, Physics.OverlapSphere global alert system, NavMeshObstacle Carving.
- **[Top-Down Shooter](https://github.com/Mike014/Top-Down-Shooter---Hotline-Miami-Mechanics)** — Hotline Miami-inspired mechanics, custom AI, physics, game state management.
- **[Survival Game Player Mechanics](https://github.com/Mike014/Survival-Game-Player-Mechanics-in-Unity)** — First-person movement, health/hunger/thirst systems, dynamic HUD.
- **[Unity 3D Survival Runner](https://github.com/Mike014/M4W4D4_Unity-3D-Prototype---Survival-Runner)** — 3D prototype focused on pure gameplay mechanics without graphic distractions.
- **[Space Invaders](https://github.com/Mike014/SpaceAttackers_or_-SpaceInvaders-xD)** — Classic arcade mechanics rebuilt in Unity.
- **[Pong](https://github.com/Mike014/Pong)** — Classic mechanics extended with custom C# AI logic, advanced physics, game state management.

### C# & Systems Study
- **[Unity C# Sandbox](https://github.com/Mike014/Unity-csharp-sandbox)** — C# scripting fundamentals, UI systems, Scripting API deep-dive.
- **[Unity Physics Shooting Lab](https://github.com/Mike014/Unity-Physics-Shooting-Lab)** — Raycasting, physics interactions, force application.
- **[Unity Math](https://github.com/Mike014/Unity_Math)** — Linear algebra and mathematical formulas applied in Unity.
- **[Unity in Action](https://github.com/Mike014/Unity-in-Action-Applied-Learning-Project-Joe-Hocking)** — Applied learning from Joseph Hocking's book: FPS basics, movement, physics.
- **[Unity in Action Advanced](https://github.com/Mike014/Unity-In-Action_Intermedio_Avanzato)** — Intermediate/Advanced continuation.

### Collaborative
- **[Team Ozzico](https://github.com/Mike014/team-ozzico)** — Collaborative project during Epicode Master. Focus on Git workflow and team development practices.

---

## Open Source Contributions

- **unDAW** (C++/Unreal) — Custom spatial audio and low-level DSP integration
- **OpenRA** (C#) — Legacy code refactoring and structural optimizations for RTS engine
- **NLTK** (Python) — Merged PR approved by co-creator Steven Bird
- **DocsGPT** (Python) — Enterprise Git workflow, strict linting, automated testing

---

## Education

- **Epicode Institute of Technology** — Master in Game Development, C# · Unity · Git · Sep 2025 – May 2026
- **MasAcademy** — Bachelor's in Music and Sound Design for Cinema and Videogames · 2022–2024
- **Codecademy** — Professional Certification in Computer Science, Python · 86/100 · 2024–2025

--- 

### CV Draft
- **[Link](https://docs.google.com/document/d/1wvzA3WK0p7dBfAYHPdW44HWnu2-JeofGgSEkJ4yxJ2w/edit?tab=t.0)** 

## Study Materials

**Architecture & Patterns:**
- [Design Patterns e Principi SOLID](https://docs.google.com/document/d/1YQPo3Dr8Qy_6aSPV4SrY-Cnk6ipnr6mMbsB4niGOinU/edit)
- [Game Programming Patterns](https://docs.google.com/document/d/1VY7C_JyBxyDLap7GBclVOt6G-BQnRX29qGwYHF3sOJM/edit)
- [Concetti di Refactoring](https://docs.google.com/document/d/1DlxRpxaOsfo4s1WO_LmzBDRcKCjY7N5qP1v55AUj6f0/edit)
- [Grokking Algorithms](https://docs.google.com/document/d/1pnhVYDMVJdLdMxqGDlwKNrJGaz3Ef80LXPnCxx3Olzo/edit?tab=t.0)
- [LeetCode](https://docs.google.com/document/d/10C1__tXVkc1cZXhjAnrs57BuKSOIKWz1IHMwY_4dQ4c/edit?tab=t.0#heading=h.26j5wc9pl6es)

**C# & .NET:**
- [C# Learn Microsoft - Tipi](https://docs.google.com/document/d/1qoFvHy1nYT9KljxUmESGIbNXwcDgCAiXwh2DJHgeODk/edit)
- [C# Learn Microsoft - Operatori Chiave](https://docs.google.com/document/d/1CqCHzytZ5Bmw147_G_tQ4ruqrOIL3cq3pujlb6B_GyM/edit)
- [Exploring Generics, Delegates and Beyond](https://docs.google.com/document/d/1sTVRDa7GyNmQft0OVPH2aJkAVdVRw28BTwQ37YwSmys/edit)
- [Argomento Avanzato C#](https://docs.google.com/document/d/1G4VHsKtjE4k4R1Tfatc13L-BXgutcK8y_7Z0bcDMmi8/edit)
- [Learn Coding Design Patterns with C# and Unity 6](https://docs.google.com/document/d/1gAY_ajA_qjzfC2e-jYfK0pJQXqGPa5upv2P1kdjC-OE/edit)
- [.NET](https://docs.google.com/document/d/1TjCKUn1dyN3qnl2b5Re-iMZcnFMmm55xLuOISUsmGIc/edit?tab=t.0#heading=h.s98uvw16p2mc)

**Unity Technical:**
- [Ottimizzare Prestazioni in Unity](https://docs.google.com/document/d/1YqE_lrGKEEZC1aK1feE7Gag8I4ra5T_NM_AHN7sRzi8/edit)
- [Saving, Loading and Serializing Data](https://docs.google.com/document/d/1tMUMo5g2LiNgOWiE8dL7fykqhwLkTp0gRWD6cXuVRas/edit?tab=t.0#heading=h.73ahb9gyhw00)
- [NavMesh](https://docs.google.com/document/d/1NLhRqfKIM5A_lw0TQd3nOaOrecXuUhBf2AToVOOavwc/edit)
- [Ottimizzazione, Ink, Testing e Build](https://docs.google.com/document/d/1WBUVNtKt5LSdpvKVw3tvr7cWjKSpea4VXdg0kBazFuE/edit)
- [LeetCode Notes](https://docs.google.com/document/d/10C1__tXVkc1cZXhjAnrs57BuKSOIKWz1IHMwY_4dQ4c/edit)

**Audio & Spatial:**
- [Spatial Audio](https://docs.google.com/document/d/1FlK848Y9hrPyqrTTwKmPOR768kCXK8g5gLvKWfBgjpA/edit)
- [Wwise Documentation](https://github.com/Mike014/Game-Development-with-Epicode/tree/main/WwiseDocumentation)

**Game Design:**
- [Game Design](https://docs.google.com/document/d/1k-Fmwk5nnLDnqpaX6phwRGkJp-L9Ygk5GnfUHYDc58Y/edit)
- [Framework Integrato Truby + Schell](https://docs.google.com/document/d/191BMukuMkgpg-7G-FGIqKKgee7paRBBeCRgZaNqaG3s/edit)
- [Anatomia Storia](https://docs.google.com/document/d/1aVspSaud5TjstIaDOu9VZmrCiat8Big41mZuVeeSjeQ/edit)

---

## Target Roles

Software Engineer · Backend Developer · Gameplay Programmer · Tools Programmer 

**Location:** Remote EU · Open to relocation 

---
